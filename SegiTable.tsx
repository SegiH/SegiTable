...
const SegiTable = ({ addtlPageSizes, cancelEditCallBackHandler, defaultPageSize, height, isAdding, isEditing, pageSizeOverride, saveAddCallBackHandler, saveEditCallBackHandler, setIsAdding, setIsEditing, showDisabled, tableTemplate, width }: Props) => {
     ...
     const validateTableComponent = (tableTemplate: ITableComponent) => {
          ...
          newTableComponent.Fields.map((field: ITableComponentField, index: number) => {
               ...
               if (!field.IsIDColumn) {
                    ...
                    if (field.FieldType !== FieldTypes.TEXTFIELD && field.FieldType !== FieldTypes.SELECT && field.FieldType !== FieldTypes.CHECKBOX) {
     ...
     return (
          <div className={`${styles.SegiTable}`}>
               ...
               {currentTableComponent && !isError && filterTableData &&
                    <span>
                         {/* Export to Excel icon */}
                         {!isError && tableData && tableData.length > 0 && tableTemplate.AllowExportCSV && !isAdding && !isEditing && currentTableComponent.AllowExportCSV &&
                         ...
                         {!isError && tableData && tableData.length > 0 && !isAdding && !isEditing && currentTableComponent.Searchable &&
                              <input className={`${!tableTemplate.AllowExportCSV && styles.SegiTableMarginTop15} ${styles.SegiTableInputStyle}`} value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} placeholder="  Search"></input>
                         }
                                                                                ...
                                                                                .map((field: ITableComponentField, index: number) => {
                                                                                     return (
                                                                                          <td key={index} className={`${styles.SegiTableDataCell} ${field.IsIDColumn ? `${styles.SegiTableIDColumn}` : ""}`}>
                                                                                               ...
                                                                                               {isEditing && field.FieldType === FieldTypes.SELECT && field.Disabled === true &&
                                                                                                    field.SelectData?.filter((currentItem) => { return currentItem[field.SelectDataIDColumn] === currentRow[field.SelectDataIDColumn] }).length > 0 &&
                                                                                                    <>
                                                                                               ...
                                                                                               {isEditing && field.FieldType === FieldTypes.CHECKBOX && field.Disabled !== true &&
